...
def seed(db: Session) -> None:
    for tid, name, tactic in TECHNIQUES:
        if not db.get(AttackTechnique, tid):
            db.add(AttackTechnique(id=tid, name=name, tactic=tactic, description=""))
    ...
    db.add(
        Notification(
            user_id=analyst.id,
            ntype="critical_alert",
            title="CRITICAL ip: 185.220.101.47",
            body="Unassigned high-severity IOC with internal authentication sightings.",
            link="/soc",
        )
    )
    db.commit()
